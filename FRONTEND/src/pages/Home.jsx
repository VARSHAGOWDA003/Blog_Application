...
const Home = () => {
  ...
  //Returns a tuple of the current URL's URLSearchParams and a function to update them. 
  ...
  const fetchBlogs = async ({ pageParam = 0 }) => {
    const response = await axiosInstance.get(`/blog/public/`, {
      params: {
        ...
        limit: PAGE_LIMIT, //blogs per page
      },
    });
    return {
      ...response.data,
      prevParam: pageParam, // current page
    };
  };

  //blog/public?q=react&page=1&limit=10

  //useinfinitequery = usequery + pagination
  const {
    data: blogsData,
    ...
    hasNextPage,
    isFetchingNextPage,
  } = useInfiniteQuery({ //get request with infinite scroll or pagination
    queryKey: ["blogs", searchQuery], //searchtext
    queryFn: fetchBlogs,
    getNextPageParam: (lastPage) => {
      const prevPage = lastPage.prevParam;
      if (lastPage.data.totalPages === 0) { //if no blogs found
        return undefined;
