...
const Profile = () => {
  ...
  const [userImg, setUserImg] = useState(null); //it holds the profile pic url already stored
  ...
  const { data: USER_PROFILE_RESPONSE, isLoading: USER_PROFILE_LOADING } = //WHATEVER USER WILL GIVE DATA THAT WILL BE STORED HERE
    useQuery({ //PROCESSING GET REQUEST 
      queryKey: ["user-profile-info"],
      queryFn: fetchProfile,
      enabled: sessionStorage.getItem("profile") ? true : false, //enabled-attribute
    }); //true-use query for refetching 
  ...
  const handleProfileClick = () => {
    console.log(PROFILE_INPUT_REF.current); //current value of useref
  ...
  const handleImageChange = (e) => {
    ...
    if (file) {
      setUserImg("") 
      setProfileImg(file); // Store the preview URL ,path of the image
    }
  };
      ...
      <article className="w-full max-w-[400px] rounded-2xl shadow p-3">
        ...
        >
          <input
            type="file"
            className="hidden"
            ref={PROFILE_INPUT_REF} //useref variable
            accept="image/*"
