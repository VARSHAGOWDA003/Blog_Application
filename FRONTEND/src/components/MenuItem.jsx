...
const MenuItem = ({ className = "" }) => {
  const location = useLocation(); //to get 
  const menuActiveBg ="bg-orange-600 text-white  "; //css part for active menu item
  ...
  const handleLogoutClick =()=>{
     sessionStorage.removeItem("profile");// removing profile from session storage
  ...
  return (
    ...
    >
      {menuData.map((menuItem, idx) => {
        ...
        if(!menuItem.secure) //checking for false secure i.e public route
        {
          return (
            <div
